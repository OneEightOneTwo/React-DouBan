  ...
  btnClick() {
    ...
    this.setState(
      prevState => ({
        fileList: [
          ...prevState.fileList,
          [prevState.inputValue, prevState.result]
        ],
    ...
    return (
      <ul>
        {this.state.fileList.map((item, index) => {
          return (
            <div className={styles.listLi} key={item}>
              <img
                className={styles.li_selfImg}
                alt=""
                src={require("../../assets/yao-1.jpg")}
              />
              <span className={styles.li_user} style={{ marginLeft: "10px" }}>
                {this.state.selfName}说:
              </span>
              {/* 文字渲染 */}
              <div className={styles.li_speak} style={{ paddingLeft: "80px" }}>
                {item[0]}
              </div>
              {/* 图片渲染 */}
              <div className={styles.listLiImg}>
                {item[1].map((it, idx) => {
                  return (
                    <img
                      className={styles.li_listImg}
                      key={idx}
                      alt=""
                      src={require(`../../assets/` + it.name)}
                    />
                  );
                })}
              </div>
